...
(function () {
  ...
  function formatAssistantMarkdown(rawText) {
    ...
    html = html.replace(/\n\n+/g, '<br><br>').replace(/\n/g, '<br>');
    // Clean up excessive leading/trailing breaks
  ...
  function formatEngineeringResponse(title, conclusion, evidenceLines, whyExplanation, missionImplication, replayTag) {
    const replayPrefix = replayTag ? `<strong>[REPLAY ${replayTag}]</strong><br><br>` : '';
    const evidenceHtml = evidenceLines.map(line => `• ${line}`).join('<br>');
    const missionHtml = missionImplication ? `<br><br><strong>Mission implication:</strong><br>${missionImplication}` : '';
    return `${replayPrefix}<strong>${title}</strong><br><br>${conclusion}<br><br><strong>Evidence:</strong><br>${evidenceHtml}<br><br><strong>Why:</strong><br>${whyExplanation}${missionHtml}`;
  }
