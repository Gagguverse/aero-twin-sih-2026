...
function getProjectRoot() {
  if (process.env.AERO_TWIN_ROOT && fs.existsSync(path.join(process.env.AERO_TWIN_ROOT, 'index.html'))) {
    return process.env.AERO_TWIN_ROOT;
  }
...
function serveStaticFile(req, res, parsedUrl) {
  ...
  fs.stat(filePath, (err, stats) => {
    ...
    stream.on('error', (streamErr) => {
      console.error('[Static Stream Error]:', streamErr.message);
      if (!res.headersSent) {
...
async function handleRequest(req, res) {
  try {
    ...
    // 5. Static File Serving: GET / or file assets
